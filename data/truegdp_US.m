...
[num,txt,~] = xlsread([dir_in 'vintagesGDP.xls'],'Vintages Starting 1999-01-29') ;
counter_q = 1 ; 
for y = years 
    for q = quarters
       truegdp_strct.quarters{counter_q} = [num2str(y) 'Q' num2str(q)] ; 
       index_row = find(strcmp(txt(2:end,1),datestr(datenum([y,q*3-2,1]),'dd.mm.yyyy'))==1) ; 
       [index_col_first, index_col_second, index_col_final] = f_find_indexcols(num,index_row) ; 

       truegdp_strct.first(counter_q) = log(num(index_row,index_col_first)) - log(num(index_row-1,index_col_first)) ; 
       truegdp_strct.second(counter_q) = log(num(index_row,index_col_second)) - log(num(index_row-1,index_col_second)) ;
       truegdp_strct.final(counter_q) = log(num(index_row,index_col_final)) - log(num(index_row-1,index_col_final)) ;
       
       counter_q = counter_q + 1 ; 
    end
end

save([dir_out 'truegdpUS.mat'],'truegdp_strct')
